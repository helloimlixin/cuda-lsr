...
def init_dist():
    if "RANK" in os.environ:
        ...
        try:
            ...
            # Older torch distributed signature.
...
# ============================================================================
# PyTorch 2.8 DeviceMesh compatibility
# ============================================================================
#
# Meta Distributed Shampoo switched on 2026-02-10 from the old
#
#     _mesh_resources._get_all_submeshes(device_mesh, mesh_dim_name)
#
# API to
#
#     device_mesh._get_all_submeshes(mesh_dim_name)
#
# PyTorch 2.8 still exposes the former but not necessarily the latter.
# Bridge only this missing method, preserving the old PyTorch implementation.
#
def install_torch28_device_mesh_compat():
    from torch.distributed.device_mesh import DeviceMesh, _mesh_resources

    if hasattr(DeviceMesh, "_get_all_submeshes"):
        return False
    ...
    if old_fn is None:
        raise RuntimeError(
            "Neither DeviceMesh._get_all_submeshes nor "
            "_mesh_resources._get_all_submeshes exists. "
            "This script's PyTorch-2.8 compatibility shim cannot apply."
        )

    def _compat_get_all_submeshes(
        self,
        mesh_dim_name,
    ):
        return old_fn(
    ...
    DeviceMesh._get_all_submeshes = (
        _compat_get_all_submeshes
    )

    return True


_TORCH28_MESH_COMPAT_INSTALLED = (
    install_torch28_device_mesh_compat()
)
...
def main():
    ...
    p.add_argument(
        ...
        default="float16",
        help="DDP Shampoo ZeRO-1 communication dtype.",
    )
    ...
    comm_dtype = (
        torch.float16
        if args.communication_dtype == "float16"
        else torch.float32
    )

    r0(
        rank,
        f"Shampoo DDP sharding     : ZeRO-1 over {world_size} trainers"
    )
    r0(
        rank,
        f"communication dtype      : {args.communication_dtype}"
    )
    ...
    # Match Meta's official DDP Shampoo setup: wrap the model first,
    # then construct DistributedShampoo from the DDP parameters.
    if world_size > 1:
        ddp_model = DDP(
            model,
    ...
    optimizer = make_optimizer(
        ...
        DDPDistributedConfig=DDPDistributedConfig,
